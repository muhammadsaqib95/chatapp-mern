...
router.route('/send-message').post(userAuth, async(req, res) => {
   ...
   socket1.in(chat.users.filter(user => user._id != req.user.id)[0]._id.valueOf()).emit('new-chat', {chat});
   res.status(chat ? 200 : 404).json(chat ?? 'Chat not found')
        ...
        try {
            ...
            socket1.in(chat.users.filter(user => user._id != req.user.id)[0]._id.valueOf()).emit('receive-message', {message : chat.messages[chat.messages.length - 1], chatId})
